...
image_angle += rotation
	...
	if alarm[1] < 30 and barrel = "primary"
	{
		image_index = 1
	}
	else if alarm[1] < 30 and barrel = "secondary"
	{
		image_index = 2
	}

	
	if alarm[1] <= 1 and _facing_player = true
		{
		image_index = 0
		var _barrel_offset = 6
		if barrel = "primary"
		{
			barrel = "secondary"
			var _offset_x = x + sin(degtorad(image_angle+180))*_barrel_offset
			var _offset_y = y + cos(degtorad(image_angle+180))*_barrel_offset
		}
		else
		{
			barrel = "primary"
			var _offset_x = x + sin(degtorad(image_angle))*_barrel_offset
			var _offset_y = y + cos(degtorad(image_angle))*_barrel_offset
		}
